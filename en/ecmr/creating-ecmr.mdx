...
## eCMR Provider: Native vs TransFollow

By default, Druma uses its **native eCMR provider** — no external account or API key required. Companies already using **TransFollow** can switch to it in Settings → Integrations → eCMR provider.

<Card title="eCMR Providers" icon="right-left" href="/en/ecmr/ecmr-providers">
  How to switch between the native Druma provider and TransFollow, and what each one does.
</Card>
